...
namespace classes;
	...
	class Login{
		...
		public function signin($USER_LOGIN_VAR,$USER_PASSW_VAR){
			...
			$query = "SELECT * FROM tbl_auth  WHERE USERNAME =".$this->sql->Param('a')." AND PASSWORD=".$this->sql->Param('b')."";
			...
			if($stmt){		
				...
				$this->setLog("Login","Login into the system");
				...
				header('Location: ' . $this->homepage);	
                                    ...
                                    $this->setLog("System denied access to $_SESSION[name]  with wrong credentials  " );
					$this->logout("wrong");
	...
	public function storeAuth($userid,$login)
	  {
		...
		public function logout($msg="out")
                {
                    $this->setLog( $this->session->get[name] ." has logout   from the system  " );
	...
	public function getMac(){
      ...
      public function setLog($event,$activity){
		$userid=$this->session->get("pyuserid");
          //$stmt = $this->connect->Execute("INSERT INTO tbl_system_log (`USERNAME`, `EVENT_TYPE`, `ACTIVITIES`, `HOSTNAME`, `IP`, `BROWSER_VERSION`,MAC_ADDRESS) VALUES ('".$userid."','".$activity."','".$this->remoteip."','".$this->useragent."','".$this->session->getSessionID()."',CURDATE(),'1')");
          $stmt = $this->connect->Execute("INSERT INTO `tbl_system_log` ( `USERNAME`, `EVENT_TYPE`, `ACTIVITIES`, `HOSTNAME`, `IP`, `BROWSER_VERSION`,MAC_ADDRESS,SESSION_ID) VALUES ('".$userid."', '$event','$activity', '".$this->hashkey."','".$this->remoteip."','".$this->useragent."','".$this->mac_addr."','".$this->session->getSessionID()."')");
