...
$days_to_keep_backup=1000;


//$file_contents[0]='c:/total#localhost';
//$file_contents[1]='root:"":total';


list($folder_name,$server_name)=explode('#',str_replace("\n","",str_replace("\n","",$file_contents[0])));
//echo $file_contents[0];
echo $folder_name=trim($folder_name);

echo $server_name=trim($server_name);

if(!is_dir($folder_name))mkdir($folder_name,0777);				
else chmod($folder_name,0777);
...
if($restart!=-1)
{
	...
	for($count=0;$count<$table_count;$count++)
	{
		...
		if(in_array($table_name,$large_tables))$max_rows=10000;
		else $max_rows=50000;
		
		if($start<$row_count || $row_count==0)
		{
			if(($start+$max_rows)>$row_count)$end=($row_count-$start);
			else $end=$max_rows;
			...
			else
			{
				...
				if($total_rows>=$max_rows)echo '<br/>';
				
				$fp = fopen($file_name, 'w');
				for($counter=0;$counter<count($file_contents);$counter++)
				{		
					fwrite($fp, $file_contents[$counter]);
					if($total_rows>=$max_rows)echo $file_contents[$counter].'<br/>';
				}
				fclose($fp);																
			}
...
if($restart!=-1&&$count<$table_count)echo '<script>location.href="http://localhost'.$_SERVER['PHP_SELF'].'?restart=0"</script>';	
else if($restart!=-1) echo '<script>location.href="http://localhost'. $_SERVER['PHP_SELF'].'?restart=-1"</script>';
